...
/** ChatBot Service Implementation Supports streaming responses via SSE */
@Service
@Slf4j
public class ChatBotServiceImpl extends ServiceImpl<ChatMessageMapper, ChatMessage>
  ...
  private final ConcurrentHashMap<String, List<Map<String, String>>> sessionContexts =
  ...
  public SseEmitter streamChatMessage(ChatMessageRequest request, Long userId) {
    ...
    if (context.size() > MAX_CONTEXT_MESSAGES) {
      List<Map<String, String>> trimmedContext =
          new ArrayList<>(context.subList(context.size() - MAX_CONTEXT_MESSAGES, context.size()));
      sessionContexts.put(sessionId, trimmedContext);
    }

    // Get final reference for use in lambda
    final List<Map<String, String>> contextRef = sessionContexts.get(sessionId);
    ...
    new Thread(
            () -> {
              try {
                ...
                contextRef.add(Map.of("role", "assistant", "content", fullResponse.toString()));
                sessionContexts.put(sessionId, contextRef);
  ...
  private String buildSystemPrompt(Long questionId) {
    ...
    prompt.append(
        "You help users understand algorithms, debug code, and solve programming problems. ");
