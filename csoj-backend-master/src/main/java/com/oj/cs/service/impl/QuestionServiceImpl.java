...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question>
  ...
  public List<Question> getCompletedQuestions(Long userId) {
    ...
    queryWrapper.eq("status", 3); // 3 表示通过
    queryWrapper.select("distinct question_id");

    // 获取已完成题目的ID列表
    List<Long> questionIds =
        questionSubmitMapper.selectList(queryWrapper).stream()
  ...
  public List<Question> getRecommendedQuestions(Long userId, int count) {
    ...
    // 获取用户已完成的题目ID列表
    List<Question> completedQuestions = getCompletedQuestions(userId);
    Set<Long> completedQuestionIds =
        completedQuestions.stream().map(Question::getId).collect(Collectors.toSet());
