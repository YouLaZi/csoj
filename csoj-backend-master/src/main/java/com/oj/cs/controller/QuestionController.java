...
import com.oj.cs.model.dto.question.*;
...
public class QuestionController {
  ...
  public BaseResponse<QuestionVO> getQuestionVOById(long id, HttpServletRequest request) {
    ...
    // 添加调试日志
    log.info("正在查询题目ID: {}", id);
    Question question = questionService.getById(id);
    log.info("查询结果: {}", question != null ? "找到题目" : "未找到题目");
    if (question != null) {
      log.info(
          "题目详情: id={}, title={}, isDelete={}",
          question.getId(),
          question.getTitle(),
          question.getIsDelete());
    }
    if (question == null) {
      throw new BusinessException(ErrorCode.NOT_FOUND_ERROR);
    }
   ...
   * 批量导入题目
   *
   * @param questionBatchAddRequest 批量题目创建请求
  ...
  public BaseResponse<QuestionBatchAddResponse> batchAddQuestion(
      @RequestBody QuestionBatchAddRequest questionBatchAddRequest, HttpServletRequest request) {
    if (questionBatchAddRequest == null || questionBatchAddRequest.getQuestionList() == null) {
      throw new BusinessException(ErrorCode.PARAMS_ERROR, "请求参数为空");
    }

    List<QuestionAddRequest> questionList = questionBatchAddRequest.getQuestionList();
    ...
    // 记录导入结果
    QuestionBatchAddResponse response = new QuestionBatchAddResponse();
    List<QuestionBatchAddResponse.QuestionImportResult> successList = new ArrayList<>();
    List<QuestionBatchAddResponse.QuestionImportResult> failList = new ArrayList<>();
    ...
    for (int i = 0; i < questionList.size(); i++) {
      QuestionAddRequest questionAddRequest = questionList.get(i);
      QuestionBatchAddResponse.QuestionImportResult importResult =
          new QuestionBatchAddResponse.QuestionImportResult();
      importResult.setIndex(i);
      importResult.setTitle(questionAddRequest.getTitle());

      try {
        // 参数校验
        if (questionAddRequest.getTitle() == null || questionAddRequest.getContent() == null) {
          throw new BusinessException(ErrorCode.PARAMS_ERROR, "题目标题或内容不能为空");
        }
        ...
        importResult.setQuestionId(question.getId());
